...
def main(args):
    ...
    ### Milvus
    ############
    # ToDo: check DB already exist and just update if exist
    ...
    if utility.has_collection("patent_search"):
        print("Drop existing collection")
        utility.drop_collection("patent_search")
        
    milvus_db = Collection("patent_search", schema)
    
    entities = [
        allids,  # field pk
        [pair['metadata']['출원번호'] for pair in passages], # field patent_id
        [pair['title'] for pair in passages], # field title
        [pair['text'] for pair in passages], # field text
    ...
    milvus_db.create_index("embeddings", index)
    milvus_db.load()
