...
def train(opt, model, optimizer, scheduler, step):
    ...
    while step < opt.total_steps:
        ...
        for i, batch in enumerate(train_dataloader):
            ## HS - DEBUG
            # logger.info(f"DEBUG - batch info:{ batch.keys() }")
            # logger.info(f"DEBUG - data q_tokens: {tokenizer.batch_decode( batch['q_tokens'],skip_special_tokens=True )}")
            # logger.info(f"DEBUG - data k_tokens : {tokenizer.batch_decode( batch['k_tokens'] ,skip_special_tokens=True)}")
            # for q,k in zip(batch['q_tokens'],batch['k_tokens']):
            #     logger.info(f"DEBUG - data pairs : {tokenizer.decode(q ,skip_special_tokens=True)} / {tokenizer.decode(k ,skip_special_tokens=True)}")
...
if __name__ == "__main__":
    ...
    # print("##### DEBUG - dist.get_rank():",dist_utils.get_rank(), "directory_exists:",directory_exists, "dist.is_initialized():",dist.is_initialized(),"dist_utils.is_main():",dist_utils.is_main(),"params.local_rank:",opt.local_rank)
